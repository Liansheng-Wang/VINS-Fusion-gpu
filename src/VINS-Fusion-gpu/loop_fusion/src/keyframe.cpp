...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	TicToc t_match;
		...
		if (DEBUG_IMAGE)    
	    {
	        ...
	        cvtColor(gray_img, loop_match_img, cv::COLOR_GRAY2BGR);
	        for(int i = 0; i< (int)point_2d_uv.size(); i++)
	        {
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            putText(notation, "current frame: " + to_string(index) + "  sequence: " + to_string(sequence), cv::Point2f(20, 30), cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);

	            putText(notation, "previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
